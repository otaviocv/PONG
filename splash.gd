...
func _process(delta):
	...
	if stages == 0:
		...
		get_node("Splash1").set_opacity((sin(time + 3*PI/2)+1)*0.5)
		print(sin(time + 3*PI/2))
		if sin(time+3*PI/2) >= 0.99:
			stages += 1
			time = 0
	elif stages == 2:
		...
		get_node("Splash2").set_opacity(sin(time/1.5))
		if sin(time/1.5) > 0.99:
			stages += 1
			time = 0
	elif stages == 5:
		stages += 1
		...
		get_node("Splash2").set_opacity(cos(time))
		if cos(time) < 0:
			stages += 1
			time = 0
